...
public:
     ...
     * @param event_base the event-base to use for this Thread (INVALID: select it automatically)
     * @param stackaddr the stack-address (0 = create a stack)
     * @param utcb the utcb-address (0 = select it automatically)
     */
    static Reference<LocalThread> create(cpu_t cpu, capsel_t event_base = INVALID,
                                         uintptr_t stackaddr = 0, uintptr_t utcb = 0) {
        ...
        // in the future :)
        return Reference<LocalThread>(new LocalThread(cpu, event_base, stackaddr, utcb));
    }
